...
<<<<<<< HEAD
                'approved_jobs_count': approved_jobs.count(),  # For Workspace Card
                'approved_jobs': approved_jobs,                # For workspace details page
            }
...
def contact_view(request):
    if request.method == 'POST':
        ...
        issue_type = request.POST.get('issue-type')
        message = request.POST.get('message')

        # Construct the email message
        email_subject = f'New Contact Submission: {issue_type}'
        email_message = (
            f'Name: {name}\n'
            f'Email: {email}\n'
        ...
        send_mail(
            ...
            ['your_recipient_email@example.com'], # To email(s)
            fail_silently=False,
        )

        return redirect('contact_success')  # Redirect to a success page

    return render(request, 'freelancer_platform/contact.html')



def contact_success_view(request):
    """
    Renders a simple success page after the contact form is submitted.
    """
    return render(request, 'freelancer_platform/contact_success.html')
=======
